...
namespace hashcode
{
	public class SmallProgram
	{
		...
		static int slices = 0;
		static int i = 0;
		static int[,] sliceSaver = new int[21, 4];
		...
		public static void Main (string[] args)
		{
			...
			StreamReader sr = new StreamReader (@"small.in");
			...
			// 1x5 slices
			int slicesCut = slicer (0, 0, 0, 4, 1, 5);
			slices = slicesCut + slices;
			score = slicesCut * 5;
			Console.WriteLine (slices + " " + score + "pts");

			// 4x1 slices
			slicesCut = slicer (0, 0, 3, 0, 1, 4);
			slices = slicesCut + slices;
			score = score + slicesCut * 4;
			Console.WriteLine (slices + " " + score + "pts");
		...
		public static void documenter(){
			...
			for (int e=0;e<slices;e++) {
				...
				using (StreamWriter sw = new StreamWriter ("small_output.txt", true)) {
					if (isFirstTime) {
						sw.WriteLine (slices);
						isFirstTime = false;
					}
